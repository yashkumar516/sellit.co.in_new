<?php 
    ...
    $output = new mobbrand();
    echo $output->getbrand(mysqli_query($con,"SELECT * FROM `product` WHERE `status` = 'active' AND `subcategoryid` = '$bid' AND `categoryid` = '$cid' AND `childcategoryid` = '$sid' ORDER BY `modify_date` DESC"),$bid);
 }else{
         $list = [
                'status' => '0',
