<?php 
    ...
    if(isset($_POST['sid']) && !empty($_POST['sid'])){
        ...
        echo $output->getbrand(mysqli_query($con,"SELECT * FROM `product` WHERE `status` = 'active' AND `subcategoryid` = '$bid'  AND `childcategoryid` = '$sid' ORDER BY  `counter` DESC, `modify_date` DESC"),$bid,$publicUrl);
 
    } else{
        echo $output->getbrand(mysqli_query($con,"SELECT * FROM `product` WHERE `status` = 'active' AND `subcategoryid` = '$bid' ORDER BY `counter` DESC, `modify_date` DESC"),$bid,$publicUrl);
