...
<?php
  if(isset($_POST['search'])){
      $search  = $_POST['search'];
      $query = mysqli_query($con,"SELECT * FROM `product` WHERE `product_name` LIKE '%$search%' AND `categoryid` = '4' ");
      ...
      if($row > 0){
          while($armodal = mysqli_fetch_assoc($query)){
            ?>
<a href="earpodsold.php?id=<?php echo $armodal['id'] ?>&&bid=<?php echo $armodal['subcategoryid'] ?>">
    <li class="py-2"><?php echo $armodal['product_name'] ?></li>
</a>
<?php
          }
      }else{
        echo "<li>No ear bud found</li>";
      }
  }
?>