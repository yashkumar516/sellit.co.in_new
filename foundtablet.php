...
<?php
  if(isset($_POST['search'])){
      $search  = $_POST['search'];
      $query = mysqli_query($con,"SELECT * FROM `product` WHERE `product_name` LIKE '%$search%' AND `categoryid` = '3' ");
      ...
      if($row > 0){
          while($armodal = mysqli_fetch_assoc($query)){
            ?>
<a href="tabletsold.php?id=<?php echo $armodal['id'] ?>&&bid=<?php echo $armodal['subcategoryid'] ?>">
