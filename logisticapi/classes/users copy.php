...
class Users
{
    ...
    {
        $usr_obj = $this->conn->prepare(
            "SELECT * FROM vendors where email = ? && password = ?"
        );
    ...
    {
        $usr_obj = $this->conn->prepare(
            "SELECT * FROM ajents where email = ? && password = ?"
        );
        $usr_obj->bind_param("ss", $this->email, $this->password);
        if ($usr_obj->execute()) {
            $data = $usr_obj->get_result();
    ...
    {
        $leadobj = $this->conn
            ->prepare("SELECT *  FROM enquiry INNER JOIN vendors on  enquiry.vendor_id  = vendors.id
          INNER JOIN address on address.enquid = enquiry.id
          INNER  JOIN address1 on address1.id = address.addressid 
          where enquiry.vendor_id = ? && enquiry.status = ? ORDER BY enquiry.id DESC");

        GENERATELOGS_API($leadobj, "[LEADS QUERy ===]", 1);
    ...
    public function get_ajent_leads()
    {
        $leadobj = $this->conn
          ...
          INNER JOIN address on address.enquid = enquiry.id
          INNER  JOIN address1 on address1.id = address.addressid 
          where vendor_id = ? && ajentid = ? && enquiry.status = ? ORDER BY enquiry.id DESC");
        $leadobj->bind_param(
            "sss",
            $this->vendorid,
            $this->ajentid,
    ...
    public function lead_description()
    {
        ...
        $leadobj = $this->conn->prepare(
            "SELECT *  FROM `enquiry` LEFT  JOIN `userrecord` on userrecord.id = enquiry.userid LEFT  JOIN `useraccount` on useraccount.enquiryid = enquiry.id  LEFT  JOIN `address` on address.enquid = enquiry.id LEFT  JOIN `address1` on address1.id = address.addressid  where  vendor_id = ? && enquiry.id = ?"
        );
        GENERATELOGS_API($leadobj, "[lead description QUERy ===]", 1);
    ...
    {
        $lead_obj = $this->conn->prepare(
            "update `enquiry` set `status` = ? where vendor_id = ? && id = ?"
        );
        $lead_obj->bind_param(
            "sss",
            $this->status,
        ...
        if ($lead_obj->execute()) {
            $selvendor = mysqli_fetch_assoc(
                mysqli_query(
                    $this->conn,
                    "SELECT * FROM `vendors` WHERE `id` = '$this->vendorid' "
                )
            );
            if ($selvendor) {
                ...
                $updatevendor = mysqli_query(
                    $this->conn,
                    "UPDATE `vendors` SET `mainwallet` = '$updatemainbalance',`mainonhold` = '$updatemainhold' WHERE `id` = '$this->vendorid'"
                );
                if ($updatevendor) {
                    $data = "successfully";
                } else {
    ...
    public function check_ajant()
    {
        $ajant_obj = $this->conn->prepare(
            "SELECT * FROM ajents where email = ? "
        );
    ...
    {
        $ajant_obj = $this->conn->prepare(
            "SELECT * FROM ajents where email = ? AND id != ?"
        );
    ...
    {
        $ajant_obj = $this->conn->prepare(
            "DELETE FROM ajents where vendorid = ? AND id = ?"
        );
    ...
    {
        $ajant_obj = $this->conn->prepare(
            "INSERT INTO ajents(`vendorid`,`ajentname`,`email`,`password`,`phone`,`adhar`,`image`,`address`) VALUES(?,?,?,?,?,?,?,?)"
        );
    ...
    {
        $leadobj = $this->conn->prepare(
            "SELECT * FROM `ajents` where vendorid = ? && status = 'active'"
        );
    ...
    {
        $lead_obj = $this->conn->prepare(
            "update `enquiry` set `ajentid` = ? where vendor_id = ? && id = ?"
        );
        $lead_obj->bind_param(
            "sss",
            $this->ajentid,
    ...
    {
        $lead_obj = $this->conn->prepare(
            "update `ajents` set `ajentname` = ?,`email` = ?,`phone` = ?,`address` = ? where id = ? && vendorid = ?"
        );
        $lead_obj->bind_param(
            "ssssss",
            $this->name,
    ...
    {
        GENERATELOGS_API($this, "[lead schedule data PACKET]", 1);

        $lead_obj = $this->conn->prepare(
            "update `address` set `soon` = ?,`time` = ?,`day` = ?,`day1` = ?,`year` = ? ,`reason` = ? where enquid = ? "
        );
        $lead_obj->bind_param(
            "sssssss",
            $this->soon,
    ...
    {
        $lid = $this->lead_id;
        $uid = mysqli_fetch_assoc(
            mysqli_query(
                $this->conn,
                "SELECT * FROM `enquiry` WHERE `id` = '$lid' "
            )
        );
        ...
        // $amount = $uid['offerprice']+$uid['extraamount'];
        // if(!empty($this->extraamount)){
        //         $amount = $uid['offerprice']+$this->extraamount;
        //       }else{
        ...
        $accountdetail = mysqli_fetch_assoc(
            mysqli_query(
                $this->conn,
                "SELECT * FROM `useraccount` WHERE `userid` = '$customerid' && `enquiryid` = '$lid' "
            )
        );
        $accountno = $accountdetail["accountno"];
        $ifsccode = $accountdetail["ifsc"];
        $userrecord = mysqli_fetch_assoc(
            mysqli_query(
                $this->conn,
                "SELECT * FROM `userrecord` WHERE `id` = '$customerid'"
            )
        );
        ...
        if ($TransactionStatus == 1) {
            $ajant_obj = $this->conn->prepare(
                "UPDATE `enquiry` set `status`= ?,`emino`= ?,`pic1`= ?,`pic2`= ?,`pic3`= ?,`pic4`= ?,`extraamount`= ?,`aadharfront`= ?,`aadharback`= ? WHERE `id`= ? && `vendor_id`= ? "
            );
            $ajant_obj->bind_param(
                "sssssssssii",
                $this->status,
            ...
            if ($ajant_obj->execute()) {
                ...
                if (!empty($upi)) {
                    $query = mysqli_query(
                        $this->conn,
                        "INSERT INTO `customertransactions`(`lead_id`,`userid`,`amount`,`type`,`success`,`response`,`message`,`BankRRN`,`UpiTranlogId`,`UserProfile`,`SeqNo`)
                         VALUES('$lid','$customerid','$amount','UPI','$TransactionStatus','$response','$message','$BankRRN','$UpiTranlogId','$UserProfile','$SeqNo')"
                    );
                } else {
                    $query = mysqli_query(
                        $this->conn,
                        "INSERT INTO `customertransactions`(`lead_id`,`userid`,`amount`,`type`,`success`,`response`,`BankRRN`,`TransRefNo`,`BeneName`,`ActCode`)
                ...
                if ($query) {
                    ...
                    $data =
                        "update successfully but history of payment not saved";
                }
                ...
                $fields = array(
                    ...
                    "message" => "SellIt online payment receipt. 
                    https://sellit.co.in/receipt.php?eId=".$genorderid,
                );
                $curl = curl_init();
    ...
    public function leadcompleteWithoutTransaction()
    {
        $lid = $this->lead_id;
        $uid = mysqli_fetch_assoc(
            mysqli_query(
    ...
    public function leadcompleteTest()
    {
        $lid = $this->lead_id;
        $uid = mysqli_fetch_assoc(
            mysqli_query(
    ...
    {
        $ajant_obj = $this->conn->prepare(
            "UPDATE `enquiry` set `status`= ?,`failreason`= ?,`offerprice`= ?,`pic1`= ?,`pic2`= ?,`pic3`= ?,`pic4`= ?,`customerprice`= ? WHERE `id`= ? && `vendor_id`= ? "
        );
        $ajant_obj->bind_param(
            "ssssssssii",
            $this->status,
    ...
    {
        $ajant_obj = $this->conn->prepare(
            "UPDATE `enquiry` set `selfie`= ? WHERE `id`= ? && `vendor_id`= ? "
        );
        $ajant_obj->bind_param(
            "sii",
            $this->selfie,
