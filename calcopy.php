...
$sumflatvalue = $fcam + $bcam + $vol + $finger + $speak +  $charginpot +  $face + $audio + $camglass + $mobwifi + $silbtn + $batt +  $baluetooth + $vibration +  $micro +  $powder;
$offerppprice = $updatedupto - ($netdeductpercenet + $sumflatvalue + $screendeduction);
// enquiry start
if (isset($_POST['otpverify'])) {
  $mobile = $_POST['mobile'];
  $uid = $_POST['uid'];
  ...
  if ($code == $_SESSION['otp']) {
    $insertenquiry = mysqli_query($con, "INSERT INTO `enquiry` (`model_name`,`mimg`,`offerprice`,`userid`,`callvalue`,`warenty`,`age`,`touchscreen`,`screenspot`,`screenlines`,`screenphysicalcondition`,`bodyscratches`,
  `bodydents`,`sidebackpanel`,`bodybents`,`charger`,`earphone`,`boximei`,`billimei`,`front_camera`,`back_camera`,`volume`,`finger_touch`,`speaker`,`power_btn`,`face_sensor`,
  `charging_port`,`audio_receiver`,`camera_glass`,`wifi`,`silent_btn`,`battery`,`bluetooth`,`vibrator`,`microphone`) VALUES('$mobilename','$modelimg','$offerppprice','$uid','$callcell[1]','$warcell[1]',
  '$agecell[1]','$touchcell[1]','$spotcell[1]','$linescell[1]','$physicalcell[1]','$scratchcell[1]','$dentscell[1]','$sidecell[1]','$bentcell[1]','$chargercell[1]','$earphonecell[1]','$boxcell[1]','$billcell[1]','$frontcamcell[1]',
  ...
  '$vibratecell[1]','$microcell[1]') ");
    if ($insertenquiry) {
      ...
      $user = $_SESSION['user'];
      echo "<script>
      window.location.href = 'price-summary.php?vid='+$vid+'&&mid='+$mid+'&&bid'+$bid;
   </script>";
    } else {
      echo '<script>
