 	...
 	<!-- end sidebar -->
				<section role="main" class="content-body content-body-modern mt-0">
					<header class="page-header page-header-left-inline-breadcrumb">
						<h2 class="font-weight-bold text-6">Series</h2>
									...
									<div class="datatables-header-footer-wrapper">
										<div class="datatable-header">
											<div class="row align-items-center mb-3">
												<div class="col-12 col-lg-auto mb-3 mb-lg-0">
													<a href="child-category.php" class="btn btn-primary btn-md font-weight-semibold btn-py-2 px-4">+ Add Series</a>
												</div>
											</div>
										</div>
										<table class="table table-ecommerce-simple table-striped mb-0" id="datatable-ecommerce-list" style="min-width: 550px;">
											...
											<!-- fetch category details start -->
                                                 <?php
												 $fetchch = mysqli_query($con,"SELECT * FROM `childcategory` WHERE `subcatid` = '$id' ");
											...
											<!-- fetch category details end -->
												<tr>
													...
													<td><a href="#"><strong><?php echo $arch['id'] ?></strong></a></td>
													<td >
														<a href="delete_childcategory.php?id=<?php echo $arch['id'] ?>&&bid=<?php echo $id ?>"><button class="btn btn-danger">Delete</button></a>
														<a href="update_childcategory.php?id=<?php echo $arch['id'] ?>&&bid=<?php echo $id ?>"> <button class="btn btn-success">Edit series</button></a>
													</td>
                                                    <?php 
                                                     $brand = $arch['subcatid'];
                                                     $fetchbrand = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `subcategory` WHERE `id` = '$brand'"));
                                                    ?>
													<td><?php echo $fetchbrand['subcategory_name'] ?></td>
                                                    <td><?php echo $arch['childcategory'] ?></td>
													<td>
												    <?php
													$biid = $arch["id"];
													$checkstatus = $arch['status'];
													if($checkstatus == 'active')
													{
													   echo	'<button class="btn btn-success" onclick="return changestatus('.$biid.')">'.$checkstatus.'</button>';
													}
													else
													{
														echo'<button class="btn btn-danger" onclick="return changestatus('.$biid.')">'.$checkstatus.'</button>';	
													}
													?>
													</td>
												</tr>
...
<!-- status script start here -->
   ...
   {
        var sid = gid;
		var url = 'change_childcategory_status.php?id='+sid+'&&bid=<?php echo $id ?>';
