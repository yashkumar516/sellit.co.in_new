 	...
 	<!-- end sidebar -->
				<section role="main" class="content-body content-body-modern mt-0">
					<header class="page-header page-header-left-inline-breadcrumb">
						<h2 class="font-weight-bold text-6">Banner</h2>
									...
									<div class="datatables-header-footer-wrapper">
										<div class="datatable-header">
											<div class="row align-items-center mb-3">
												<div class="col-12 col-lg-auto mb-3 mb-lg-0">
													<a href="upload_banner.php" class="btn btn-primary btn-md font-weight-semibold btn-py-2 px-4">+ Add Banner</a>
												</div>
											</div>
										</div>
										<table class="table table-ecommerce-simple table-striped mb-0" id="datatable-ecommerce-list" style="min-width: 550px;">
											...
											<!-- fetch category details start -->
                                              <?php
												$fetchbanner = mysqli_query($con,"SELECT * FROM `banner`");
												while($arbanner = mysqli_fetch_assoc($fetchbanner))
												{
											?>
											<!-- fetch category details end -->
											    <tr>
													<td width="30"><input type="checkbox" name="checkboxRow1" class="checkbox-style-1 p-relative top-2" value="" /></td>
													<td><a href="#"><strong><?php echo $arbanner['id'] ?></strong></a></td>
													<td width="23%">
														<a href="delete_banner.php?id=<?php echo $arbanner['id'] ?>"><strong><i class="fas fa-trash-alt mr-3" style="font-size:20px;"></i></strong></a>
														<a href="update_banner.php?id=<?php echo $arbanner['id'] ?>"><i class="fas fa-edit ml-1" style="font-size:20px;"></i></strong></a>
													</td>
                                                      <td><img src="img/<?php echo $arbanner['banner_image'] ?>" width="100px" alt=""></td>
													  <td><a href="#"><strong><?php echo $arbanner['category'] ?></strong></a></td>
                            
													<td><?php echo date('d/m/y',strtotime($arbanner['modify_date']))  ?></td>
													<td>
													<?php
                                                    $bid = $arbanner['id'];
													$checkstatus =$arbanner['status'];
...
swal({ title: "warning",
 ...
 type: "success"}).then(okay => {
   if (okay) {
    window.location.href = 'change_banner_status.php?id='+id;
  }
  else
  {
