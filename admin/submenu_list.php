			 ...
			 display: none !important;
				...
				<section role="main" class="content-body content-body-modern mt-0">
					<header class="page-header page-header-left-inline-breadcrumb">
						<h2 class="font-weight-bold text-6">Menu</h2>
										...
										<div class="datatable-header">
											<div class="row align-items-center mb-3">
												<div class="col-12 col-lg-auto mb-3 mb-lg-0">
													<a href="upload_submenu.php" class="btn btn-primary btn-md font-weight-semibold btn-py-2 px-4">+ Add SubMenu</a>
												</div>
												<div class="col-8 col-lg-auto ml-auto mb-3 mb-lg-0">
													<div class="d-flex align-items-lg-center flex-column flex-lg-row">
														<label class="ws-nowrap mr-3 mb-0" style="display:block !important;">Filter By:</label>
														<select class="form-control select-style-1 filter-by" name="filter-by">
															<option value="all" selected>All</option>
															<!-- <option value="1">ID</option>
														
															<option value="3">Submenu Name</option>
												...
												<div class="col-12 col-lg-auto pl-lg-1">
													<div class="search search-style-1 search-style-1-lg mx-lg-auto">
														<div class="input-group">
															<input type="text" class="search-term form-control" name="search-term" id="search-term" placeholder="Search SubMenu">
										...
										<table class="table table-ecommerce-simple table-striped mb-0" id="datatable-ecommerce-list" style="min-width: 550px;">
											<thead>
												<tr>
													...
													<th width="28%">SubMenu Name</th>
													<th width="28%">SubMenu Link</th>
											...
											<!-- fetch category details start -->
                                              <?php
												$fetchsubmenu = mysqli_query($con,"SELECT * FROM `submenu`");
												while($arsubmenu= mysqli_fetch_assoc($fetchsubmenu))
												{
											  ?>
											<!-- fetch category details end -->
												<tr>
													<td width="30"><input type="checkbox" name="checkboxRow1" class="checkbox-style-1 p-relative top-2" value="" /></td>
													<td><a href="#"><strong><?php echo $arsubmenu['id'] ?></strong></a></td>
													<td>
														<a href="delete_submenu.php?id=<?php echo $arsubmenu['id'] ?>"><strong><i class="fas fa-trash-alt mr-3" style="font-size:20px;"></i></strong></a>
														<a href="update_submenu.php?id=<?php echo $arsubmenu['id'] ?>"><i class="fas fa-edit ml-1" style="font-size:20px;"></i></strong></a>
                                                    </td>
                                                    <td>
														<?php
														$menuid = $arsubmenu['menu_id'];
														$m = mysqli_query($con,"SELECT * FROM `menu_mgt` WHERE `id` = '$menuid' ");
														$fetchm = mysqli_fetch_assoc($m); 
														?>
														<?php echo  $fetchm['name'] ?>
													</td>
													<td><?php echo $arsubmenu['submenuname'] ?></td>
													<td><?php echo $arsubmenu['link'] ?></td>
													<td><?php echo date('d/m/y',strtotime($arsubmenu['modify_date']))  ?></td>
													<td>
													<?php
													$mid = $arsubmenu['id'];
													$checkstatus = $arsubmenu['status'];
													if($checkstatus == 'active')
													{
													   echo	'<button class="btn btn-success" onclick="return changestatus('.$mid.')">'.$checkstatus.'</button>';
													}
													else
													{
														echo'<button class="btn btn-danger" onclick="return changestatus('.$mid.')">'.$checkstatus.'</button>';	
													}
													?>
													</td>
												</tr>
...
<!-- status script start here -->
   ...
   {
        var id = gid;
		var url = 'change_submenu_status.php?id='+id;
