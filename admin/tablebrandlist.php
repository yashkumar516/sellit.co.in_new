								...
								<div class="card-body">
									<div class="datatables-header-footer-wrapper">
										<div class="datatable-header">
											<div class="row align-items-center mb-3">
												<div class="col-12 col-lg-auto mb-3 mb-lg-0">
													<a href="tabletbrand.php" class="btn btn-primary btn-md font-weight-semibold btn-py-2 px-4">+ Add Brand</a>
												</div>
										...
										<table class="table table-ecommerce-simple table-striped mb-0" id="datatable-ecommerce-list" style="min-width: 550px;">
											<thead>
												<tr>
													<th width="1%"><input type="checkbox" name="select-all" class="select-all checkbox-style-1 p-relative top-2" value="" /></th>
													<th width="3%">ID</th>
													<th width="23%">Action</th>
													<th width="12%">Category Id</th>
													<th width="12%">Brand</th>
													<th width="11%">Image</th>
													<th width="12%">Last Modify</th>
													<th width="10%">Status</th>
													<th width="12%">Top Selling</th>
											...
											<!-- fetch category details start -->
                                              <?php
												$fetchsub = mysqli_query($con,"SELECT * FROM `subcategory` WHERE `category_id` = '3'");
												while($arsub = mysqli_fetch_assoc($fetchsub))
												{
													$catid = $arsub['category_id'];
													$sid = $arsub['id'];
													$catname = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `category` WHERE `id` = '$catid' "));
													$childcat = mysqli_num_rows(mysqli_query($con,"SELECT * FROM `childcategory` WHERE `subcatid` = '$sid' AND `categoryid` = '$catid'"));
											  ?>
											<!-- fetch category details end -->
												<tr>
													...
													<td><a href="#"><strong><?php echo $arsub['id'] ?></strong></a></td>
													<td>
														<a href="delete_tablebrand.php?id=<?php echo $arsub['id'] ?>"><strong><i class="fas fa-trash-alt mr-3" style="font-size:20px;"></i></strong></a>
														<a href="update_tablebrand.php?id=<?php echo $arsub['id'] ?>"><i class="fas fa-edit ml-1" style="font-size:20px;"></i></strong></a>
														<?php
                                                        if($childcat >= 1){
														?>
														<a href="series-list.php?id=<?php echo $arsub['id'] ?>"><button class="btn btn-warning ml-3">Update Series</button></a>
														<?php
														}
														?>
													</td>
													...
													<td><a href="#"><img src="img/<?php echo $arsub['subcategory_image'] ?>" alt="img" width="100px"></a></td>
													<td><?php echo date('d/m/y',strtotime($arsub['modify_date']))  ?></td>
													<td>
													<?php
													$sid = $arsub['id'];
													$checkstatus = $arsub['status'];
													if($checkstatus == 'active')
													{
													   echo	'<button class="btn btn-success" onclick="return changestatus('.$sid.')">'.$checkstatus.'</button>';
													}
													else
													{
														echo'<button class="btn btn-danger" onclick="return changestatus('.$sid.')">'.$checkstatus.'</button>';	
													}
													?>
													</td>
													<td>
													...
													if($checktop == 'active')
													{
													   echo	'<button class="btn btn-success" onclick="return changetop('.$tid.')">'.$checktop.'</button>';
													}
													else
													{
														echo'<button class="btn btn-danger" onclick="return changetop('.$tid.')">'.$checktop.'</button>';	
													}
													?>
													</td>
												</tr>
...
<!-- status script start here -->
   ...
   {
        var id = gid;
		var url = 'change_tablebrand_status.php?id='+id;
		$(location).attr('href',url);
   }
   </script>
    <script>
   function changetop(gid)
   {
        var id = gid;
		var url = 'change_tablebrand_top.php?id='+id;
		$(location).attr('href',url);
   }
   </script>
